...
export function buildPromptMessages(input: BuildPromptMessagesInput): ChatMessage[] {
  const memoryBlock = input.memoryContext.results.length > 0
    ? input.memoryContext.results
        .map(
          (result, index) => `[MEM_${index + 1}] ${truncate(result.memory.content, CHAT_MEMORY_CONTEXT_CHAR_LIMIT)}\nsourceArtifactId=${result.memory.sourceArtifactId}`,
        )
        .join("\n\n")
  ...
  return [
    {
      ...
      content: [
        ...
        "If the user asks for personal memory and the provided core comms or memory context does not contain the answer, say you do not have that memory yet; keep it concise, lightly cheeky, and reference the user's wording without inventing a fact.",
