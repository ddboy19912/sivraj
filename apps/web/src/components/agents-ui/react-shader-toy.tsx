...
const VALID_PRECISIONS = new Set(PRECISIONS);
const GLSL_IDENTIFIER_PATTERN = /[A-Za-z_][A-Za-z0-9_]*/g;
...
function collectShaderIdentifiers(source: string) {
  return new Set(source.match(GLSL_IDENTIFIER_PATTERN) ?? []);
}
...
function useShaderToyCanvas({
  ...
  const animateWhenNotVisibleOnMountRef = useRef(animateWhenNotVisible);
  ...
  const preProcessFragment = (fragment: string) => {
    const isValidPrecision = VALID_PRECISIONS.has(precision);
    ...
    const shaderIdentifiers = collectShaderIdentifiers(fragment);
    for (const uniform of Object.keys(uniformsRef.current)) {
      if (shaderIdentifiers.has(uniform)) {
  ...
  const drawScene = (timestamp: number) => {
    ...
    if (animateWhenNotVisibleOnMountRef.current || isVisibleRef.current) {
  ...
  useEffect(() => {
    if (animateWhenNotVisibleOnMountRef.current || !canvasRef.current) return;
    ...
    // drawScene reads refs owned by the WebGL loop; rebuilding this observer on every render jitters the canvas.
    // eslint-disable-next-line react-hooks/exhaustive-deps, react-doctor/exhaustive-deps
  }, []);

  // Main effect for initialization and cleanup
  // WebGL setup is mount-scoped; dynamic shader props flow through refs without reinitializing the context.
...
export function ReactShaderToy({
  ...
  const canvasRef = useShaderToyCanvas({
    fs,
    vs,
    textures,
    propUniforms,
